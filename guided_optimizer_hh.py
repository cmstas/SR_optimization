...
class Guided_Optimizer():
    def __init__(self, **kwargs):
        ...
        # how many epochs of optimization to run (-1 means run until convergence)
    ...
    def generate_cut_combos(self, N_combos, mvas, n_bin, mode):
        ...
        if self.diagnostic_mode:
          if self.extSRs is None:
            X = self.generate_random_cut_combos(1,mvas,n_bin)
        
          else:
            X = self.generate_fixed_cut_combos(self.extSRs,mvas,n_bin)
    ...
    def generate_fixed_cut_combos(self, extSRs, mvas, n_bin):
        if self.verbose:
            print("[GUIDED_OPTIMIZER] Applying provided cuts for %d bins with mvas" % n_bin, mvas)

        X = []
        
        if len(extSRs) != (n_bin * len(mvas)):
              raise ValueError("[GUIDED_OPTIMIZER] provided boundaries {} and n_bin {} x mvas {} don't match".
                               format(extSRs, n_bin, mvas))
        X.append(extSRs)
    ...
    def generate_random_cut_combos(self, N_combos, mvas, n_bin):
        ...
        for i in range(N_combos):
            if i < 3 and len(mvas) == 1 and n_bin == 1:
                ...
                cuts_list = self.convert_eff_to_cut(mvas[0], effs_list[:n_bin]) + \
                            self.convert_eff_to_cut(mvas[1], effs_list[n_bin:]) 
            if self.verbose:
                if i < 10:
    ...
    def convert_single_eff_to_cut(self, mva, eff):
        ...
        # import pdb 
        # pdb.set_trace()
        return self.quantiles[mva]["scores"][idx]
    ...
    def calculate_expected_limit(self, selection, idx, m_point, temp_results):
        ...
        disqualify_srs = False # disqualify certain binning combinations if they don't have enough non-res bkg events 
                               # in mgg sidebands (user can specify how many)
        ...
        for i in range(len(selection)):
            ...
            if bkg_yield_raw < self.minSBevnts or bkg_yield_raw_data < self.minSBevnts:
                print("[GUIDED OPTIMIZER] Only %.6f (%.6f) expected background events from MC (data) in SB, disqualifying signal region set." % (bkg_yield_raw,bkg_yield_raw_data) )
                disqualify_srs = True
        ...
        combineConfig = {
                ...
                "cardName" : "CMS-HGG_mva_13p6TeV_datacard_" + str(idx) + ".txt",
        ...
        if disqualify_srs and not self.diagnostic_mode:
            exp_lim *= 3 # triple the expected limit if the SR combination is disqualified bc too few non-res bkg events
            # the reason we triple the exp_lim, is that we still want the expected limit to be a relatively smooth 
            # function of the cut values. this way, the optimization bdt can hopefully learn that cut values resulting 
            # in very narrow bins have a penalty applied on them
        ...
        for tag in tagList:
            ...
            combineConfig["cardName"] = "CMS-HGG_mva_13p6TeV_datacard_" + str(idx) + "_" + tag + ".txt"
